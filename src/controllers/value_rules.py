...
def allowed_values(
    ...
    Check, if standardized shapefiles contain permissible values.
    If zip file contains shapefiles that has unknown name, checking process
    ...
    try:
        ...
        if len(wrong_values_geom) > 0 and export_values is False:
            if verbose is True:
                print(
                    f"Error: There are features that do not respect naming convention ({len(wrong_values_geom)}):",
                    *wrong_values_info,
                    sep="\n",
                    end="\n" * 2
                )
            ...
            print(f"Ok: All features respect naming convention.",
                  end="\n" * 2)
